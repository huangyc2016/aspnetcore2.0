...
namespace HYC.Repository
{
    ...
    {
        public readonly IOptions<RedisHelper> _optionsAccessor;
        public RedisCache(IOptions<RedisHelper> optionsAccessor)
        {
            this._optionsAccessor = optionsAccessor;
        }
        private readonly string CustomKey = "HYC";

        #region 辅助方法

        private T Do<T>(Func<IDatabase, T> func)
        {
            var database = _optionsAccessor.Value.RedisDB;
        ...
        /// <param name="key">缓存key名</param>
        /// <returns>是否存在</returns>
        public async Task<bool> KeyIsExistAsync(string key)
        {
            key = AddSysCustomKey(key);
            return await Do(o => o.KeyExistsAsync(key));
        }
        ...
        /// <returns></returns>
        public T AutoRedis<T>(string key, Func<T> fun, TimeSpan? expiry = default(TimeSpan?))
        {
            if (!Do(o => o.KeyExists(AddSysCustomKey(key))))
            {
                StringSet<T>(key, fun());
            }
